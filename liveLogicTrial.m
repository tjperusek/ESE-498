...
while (j < 7)
    ...
    for k=1:dhours(j)
        ...
        if (array(index+k-1,3) == 2)
            ...
            % running duration
            if (array(index+k-1,2)+thresh_diff >= sum && array(index+k-1,2)>= devices(j))
                check(j) = 1;
    ...
    elseif (check(j) == 0)
        ...
        for k=1:dhours(j)
            if (array(index+k-1+dhours(j-1),3) == 2)
                if (array(index+k-1+dhours(j-1),2)+thresh_diff>= sum && array(index+k-1+dhours(j-1),2)>= devices(j))
                    last_check = 1;
                    if (k == dhours(j) && last_check == 1)
                        for r=1:k
